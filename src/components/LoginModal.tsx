...
const LoginModal: React.FC<LoginModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleKeycloakRegister = () => {
    const registerUrl = `${keycloakBase}/realms/${realm}/protocol/openid-connect/registrations`;
    console.info('Redirecting to Keycloak SSO registration URL:', registerUrl);
  };
    ...
    <div className="modal-backdrop" role="presentation" onClick={onClose}>
      <div className="modal" role="dialog" aria-modal="true" aria-labelledby="login-title" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="sso-panel">
          ...
          <div className="sso-hint">Current SSO endpoint target: {`${keycloakBase}/realms/${realm}/protocol/openid-connect/auth`}</div>
        </div>
        <div className="backend-hint">Backend auth base URL: {backendAuthBase}</div>
      </div>
    </div>
  );
};
